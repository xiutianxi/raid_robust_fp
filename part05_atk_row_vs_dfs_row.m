...
[chg_row,chg_col] = ind2sub(size(db),find(db~=db_mark)); % record marked locations

db_unmarked_part = db(  setdiff( ([1:row_num])'  ,  chg_row ),   : );

comm = 10; %[10:1:20];
rng(123)
affiliation = kmeans( db_unmarked_part ,comm );
gamma = 30;
...
for j  = 1:comm
    ...
    individual_of_comm_i = db_unmarked_part( individual_id ,:   );
    D_og = squareform( pdist(individual_of_comm_i,  'hamming' ) ) * 13;
    
    D_og = exp(-D_og/13)-eye(num_individual);
    
    [~,centroid,~,dis_to_cen] = kmeans(individual_of_comm_i,1);
    
        mod_num = ceil(  num_individual/(gamma*2)    ); % change this much unmarked individuals.
        
   [val,peripheral_idx ]  =     sort(dis_to_cen, 'descend');
    peripheral_idx = peripheral_idx([1:mod_num]);
    
    [uA,~,uIdx] = unique(individual_of_comm_i,'rows');
modeIdx = mode(uIdx);
modeRow = uA(modeIdx,:);

individual_of_comm_i (peripheral_idx,:) = repmat(modeRow, mod_num,1);
    ...
    if isempty(id_diff)
        ...
        sus = [sus   ;     individual_id(id_diff([1:ceil(  num_individual/(gamma*2)    )])   )   ];
    end
    j
end
...
for j  = 1:comm
    ...
    D_og =    squareform( pdist(individual_of_comm_i,  'hamming' ) ) * 13   ;
    D_og = exp(-D_og/13)-eye(num_individual);
    dist_noise = normrnd(0,sigma^2,num_individual,num_individual);
    dist_noise = (dist_noise+dist_noise')/2;
    dist_noise = dist_noise - diag(diag(dist_noise));
    D_ref =   min(  max(0, D_og+dist_noise),1);
    
    individual_of_comm_i_mark = db_mark(   find(affiliation==j),:   );
    D_mark =   squareform( pdist(individual_of_comm_i_mark,  'hamming' ) ) * 13   ;
    D_mark = exp(-D_mark/13)-eye(num_individual);
    abs_diff = abs(D_ref-D_mark) ;
    [val,id_diff] = sort(sum(abs_diff,2),'descend');
    if isempty(id_diff)
        continue
    else
